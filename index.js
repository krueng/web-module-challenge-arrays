...
function removeFlavorByName(array, flavorToRemove){
  /*your code here*/
  array.indexOf(flavorToRemove) >= 0 ? array.splice(array.indexOf(flavorToRemove), 1) : array = 'Please try again!';
  return array;
}
...
function filterByWord(array, filterWord){
  ...
  for (let i=0; i<array.length; i++){
    array[i].includes(filterWord)?filteredArray.push(array[i]):false;
  }
...
function getAverageWordLength(array){
  ...
  let totalAverage = 0;
  for (let i=0; i<array.length; i++){
    totalAverage = array[i].length + totalAverage;
  }
	return (Math.round(totalAverage/array.length));
}
...
function getRandomFlavors(...flavors){
  /*code here*/
	let allArray = [].concat(...flavors);
	let randomFlavors = [];
	let random = 0;
	// allArray.(...flavors)
	for (let i=0; i<31; i++){
		random = Math.floor(Math.random() * allArray.length);
    randomFlavors.push(allArray[random]);
  }
